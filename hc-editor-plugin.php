...
if (!class_exists('HealcodeShortcodeContainer')) {
  class HealcodeShortcodeContainer {
    ...
    public function include_hc_media_button_js() {
      wp_enqueue_script('hc_media_script', plugins_url('/js/healcode-wp-mb-widget.js', __FILE__), array('jquery'), '1.1.15', true);
    }
    ...
    public function add_hc_media_button($editor_name) {
      ...
      printf("<a href=\"#\" class=\"button hc-wp-media-button\" data-editor=\"%s\" data-hc-codes=\"%s\"><i class=\"mce-ico mce-i-icon hc-hmw-own-icon\"></i> HealCode</a>", $editor_name, htmlspecialchars($hc_json), 100);
    }
